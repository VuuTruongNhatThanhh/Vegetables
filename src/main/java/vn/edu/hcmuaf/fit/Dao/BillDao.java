...
public class BillDao {
    ...
    public String addDB(double total, String idU) {
    ...
    public List<Bills> RecentBill() {
        List<Bills> res = new LinkedList<>();
        try {
            PreparedStatement ps = DBConnect.getInstance().get("select MAHD, NGHD, TONGTIEN, TRANGTHAI, MATK from hoadon where TRANGTHAI = 0 and DATEDIFF(Date(NOW()), NGHD) = 0");
            ResultSet rs = ps.executeQuery();
            while (rs.next()) {
                res.add(new Bills(rs.getString(1), rs.getDate(2), rs.getInt(3), rs.getInt(4), rs.getString(5)));
            }
    ...
    public List<Bills> CancelBill() {
        List<Bills> res = new LinkedList<>();
        try {
            PreparedStatement ps = DBConnect.getInstance().get("select MAHD, NGHD, TONGTIEN, TRANGTHAI, MATK from hoadon where TRANGTHAI = 3");
            ResultSet rs = ps.executeQuery();
            while (rs.next()) {
                res.add(new Bills(rs.getString(1), rs.getDate(2), rs.getInt(3), rs.getInt(4), rs.getString(5)));
            }
    ...
    public List<Bills> ConfirmBill() {
        List<Bills> res = new LinkedList<>();
        try {
            PreparedStatement ps = DBConnect.getInstance().get("select MAHD, NGHD, TONGTIEN, TRANGTHAI, MATK from hoadon where TRANGTHAI = 1");
            ResultSet rs = ps.executeQuery();
            while (rs.next()) {
                res.add(new Bills(rs.getString(1), rs.getDate(2), rs.getInt(3), rs.getInt(4), rs.getString(5)));
            }
    ...
    public List<Bills> CancelBill(String id) {
        List<Bills> res = new LinkedList<>();
        try {
            PreparedStatement ps = DBConnect.getInstance().get("select MAHD, NGHD, TONGTIEN, TRANGTHAI, MATK from hoadon where TRANGTHAI = 3 and MATK = ?");
            ps.setString(1, id);
            ResultSet rs = ps.executeQuery();
            while (rs.next()) {
                res.add(new Bills(rs.getString(1), rs.getDate(2), rs.getInt(3), rs.getInt(4), rs.getString(5)));
    ...
    public List<Bills> RecentBill(String id) {
        List<Bills> res = new LinkedList<>();
        try {
            PreparedStatement ps = DBConnect.getInstance().get("select MAHD, NGHD, TONGTIEN, TRANGTHAI, MATK from hoadon where TRANGTHAI = 0 and MATK = ?");
            ps.setString(1, id);
            ResultSet rs = ps.executeQuery();
            while (rs.next()) {
                res.add(new Bills(rs.getString(1), rs.getDate(2), rs.getInt(3), rs.getInt(4), rs.getString(5)));
